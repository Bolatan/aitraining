      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row gap-8">
        <aside className="w-full md:w-80 shrink-0 space-y-2">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-3.5 rounded-xl border transition-all flex items-center justify-between group ${
                    isSelected
                      ? 'bg-white border-indigo-500 shadow-sm ring-1 ring-indigo-500/30 text-slate-900'
          ...
          <div className="pt-4 border-t border-slate-200">
            <Link
              href="/capstone"
              className="w-full p-3.5 rounded-xl bg-gradient-to-r from-amber-50 to-purple-50 border border-amber-300/60 hover:border-amber-400 text-amber-900 flex items-center justify-between transition-colors block text-xs font-semibold shadow-xs"
            >
        ...
        <main className="flex-1 bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 flex flex-col justify-between shadow-xs">
          {!user?.isAuthorized && !user?.isAdmin ? (
            <div className="my-auto py-12 text-center space-y-6 max-w-lg mx-auto">
              ...
              <div>
                <span className="text-xs font-mono uppercase tracking-widest text-amber-800 bg-amber-100 px-3 py-1 rounded-full border border-amber-200">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 text-xs text-slate-600 text-left space-y-2 font-mono">
                <p className="text-slate-900 font-semibold font-sans">How to activate access:</p>
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <div className="shrink-0 flex items-center space-x-2">
                  {selectedProgress?.completed ? (
                    <span className="px-3.5 py-1.5 rounded-lg bg-emerald-50 border border-emerald-300 text-emerald-800 text-xs font-bold flex items-center space-x-1.5 shadow-xs">
                      <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                      <span>Section Completed</span>
                    </span>
                  ) : selectedProgress?.quizPassed ? (
                    <span className="px-3.5 py-1.5 rounded-lg bg-amber-50 border border-amber-300 text-amber-800 text-xs font-bold flex items-center space-x-1.5 shadow-xs">
              ...
              <div className="space-y-6">
                ...
                {selectedModule.lessons.map((lesson, idx) => (
                  <div
                    key={idx}
                    className="p-5 rounded-xl bg-slate-50/80 border border-slate-200 space-y-2 hover:border-slate-300 transition-colors"
                  >
                    <div className="flex items-center space-x-2">
              ...
              {selectedModule.exercise && (
                <div className="p-5 rounded-xl bg-gradient-to-r from-slate-50 to-indigo-50/60 border border-indigo-200 space-y-2">
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left p-3 rounded-lg border text-xs transition-all flex items-center justify-between ${
                                  isSelected
                                    ? 'bg-indigo-100 border-indigo-500 text-indigo-950 font-semibold shadow-xs'
                    ...
                    {quizResult && (
                      <span
                        className={`text-xs font-bold font-mono ${
                          quizResult.passed ? 'text-emerald-700' : 'text-amber-800'
                        }`}
                      >
