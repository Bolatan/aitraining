...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 sm:px-6 lg:px-8 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center space-x-3 w-full sm:w-auto">
            <div className="w-10 h-10 rounded-lg bg-indigo-100 border border-indigo-200 flex items-center justify-center shrink-0">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row gap-8">
        <aside className="w-full md:w-80 shrink-0 space-y-2">
          ...
          <div className="pt-4 border-t border-slate-200 space-y-2">
            <Link
              href="/sessions"
              className="w-full p-3.5 rounded-xl bg-indigo-50/60 border border-indigo-200 hover:border-indigo-400 text-indigo-700 flex items-center justify-between transition-colors block text-xs font-semibold"
            >
            ...
            <Link
              href="/capstone"
              className="w-full p-3.5 rounded-xl bg-amber-50/60 border border-amber-200 hover:border-amber-400 text-amber-800 flex items-center justify-between transition-colors block text-xs font-semibold"
            >
              <div className="flex items-center space-x-2">
        ...
        <main className="flex-1 bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 flex flex-col justify-between shadow-sm">
          {!user?.isAuthorized && !user?.isAdmin ? (
            <div className="my-auto py-12 text-center space-y-6 max-w-lg mx-auto">
              <div className="w-16 h-16 rounded-full bg-amber-100 border border-amber-300 flex items-center justify-center mx-auto text-amber-700 shadow-xs">
                <Key className="w-8 h-8" />
              </div>

              <div>
                <span className="text-xs font-mono uppercase tracking-widest text-amber-800 bg-amber-100 px-3 py-1 rounded-full border border-amber-300 font-bold">
              ...
              <form onSubmit={handleTokenSubmit} className="space-y-3 bg-slate-50 p-5 rounded-2xl border border-slate-200 text-left">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 rounded-xl bg-white border border-slate-300 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-amber-500 uppercase tracking-wider font-mono"
                  />
                  <button
                    type="submit"
                    disabled={tokenSubmitting}
                    className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold text-xs shadow-xs transition-colors flex items-center justify-center space-x-1.5 shrink-0 disabled:opacity-50 cursor-pointer"
                  >
                ...
                {tokenError && (
                  <p className="text-xs text-rose-600 font-mono flex items-center space-x-1 pt-1">
                    <AlertCircle className="w-3.5 h-3.5 shrink-0" />
                    <span>{tokenError}</span>
                  </p>
                )}
                {tokenSuccess && (
                  <p className="text-xs text-emerald-600 font-mono flex items-center space-x-1 pt-1">
                    <CheckCircle2 className="w-3.5 h-3.5 shrink-0" />
                    <span>{tokenSuccess}</span>
                  </p>
                )}

                <div className="pt-2 text-[11px] text-slate-500 font-mono space-y-1 border-t border-slate-200 mt-3">
                  <p className="text-slate-700 font-sans font-semibold">How to activate access:</p>
            ...
            <div className="space-y-8">
              <div
                ...
                style={{
                  backgroundColor: `${selectedModule.accentColor}10`,
                  borderColor: `${selectedModule.accentColor}30`,
                }}
              >
                <div>
              ...
              {selectedModule.exercise && (
                <div className="p-5 rounded-xl bg-indigo-50/60 border border-indigo-200 space-y-2">
              ...
              {selectedModule.quiz && selectedModule.quiz.length > 0 && (
                <div className="p-6 rounded-2xl bg-slate-50 border border-slate-200 space-y-6">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-4">
                    ...
                    {selectedProgress?.quizPassed && (
                      <span className="text-xs font-bold text-emerald-800 bg-emerald-100 px-3 py-1 rounded-full border border-emerald-300 flex items-center space-x-1">
                  ...
                  <div className="pt-2 flex items-center justify-between">
                    <button
                      ...
                      className="px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-xs transition-colors disabled:opacity-50"
                    >
              ...
              <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  ...
                  className={`px-6 py-3 rounded-xl font-bold text-xs shadow-xs transition-all flex items-center space-x-2 ${
                    selectedProgress?.completed
